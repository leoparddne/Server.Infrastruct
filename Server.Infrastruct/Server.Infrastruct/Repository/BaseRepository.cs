...
using Server.Infrastruct.Helper;
        ...
        private ISugarQueryable<T> GetQueryableByCondition(List<SearchConditionItem> conditions, bool fieldNameISDBName, Expression<Func<T, bool>> expEx)
        {
            ...
            if (conditions != null)
            {
                ...
                foreach (SearchConditionItem condition in conditions)
                {
                    ...
                    ExceptionHelper.Check(sqlFieldName == null, MessageEnum.FieldNotFound);
                    ...
                    var dbType = SqlSugerEx.GetDBType();
        ...
        public static string GetValue(Type fieldType, string calcValue)
        {
            switch (fieldType.Name)
            {
                ...
                case nameof(DateTime):
                    DbType dbType;
                    if (Enum.TryParse(DBConfigSingleton.GetConfig().ConnectionType, out dbType))
                    {
                        switch (dbType)
                        {
                            case DbType.MySql:
